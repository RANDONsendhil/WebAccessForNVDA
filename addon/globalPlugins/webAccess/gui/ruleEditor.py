...
class RuleContextEditor(wx.Dialog):
    ...
    # --------------------------------------------------------------------------------------------------------------
    # Method return the current pageType for the concerned module
    # --------------------------------------------------------------------------------------------------------------
    @classmethod
    def getCurrentPageTypeName(cls, data):
        for key, label in cls.FIELDS.items():
            if key in data:
                return data[key]
        else:
    ...
    def onOk(self, evt):
        ...
        if self.checkJointureRules(markerManager, self.pageTypeCombo.Value):
            updateAndDeleteMissing(self.FIELDS, data, self.data)

        else:
            gui.messageBox("Le Nom du pageType associé avec ce Marker va être modifier par {}".format(self.pageTypeCombo.Value))
            if self.getPageTypeRuleByName(markerManager, RuleEditor.getCurrentContextPageType(self)):
                rule = self.getSelectedRuleQuery(self.objectPageType)
                updateAndDeleteMissing(self.FIELDS, data, self.data)
                RuleEditor.setPageTypeNameByObject(self, markerManager, rule, data=rule.dump(), name=RuleEditor.getCurrentContextPageType(self))

        assert self.IsModal()
        self.EndModal(wx.ID_OK)

    # --------------------------------------------------------------------------------------------------------------
    # Checks whether the marker rule's name is associated with a pagetype's nema otherwise checkJointureRules()
    # return a boolean value True which allows to update the contextpage of the marker
    # --------------------------------------------------------------------------------------------------------------
    def checkJointureRules(self, markerManager, newValuePageContext):
        for rules in markerManager.getRules():
            if rules.name == newValuePageContext:
                return True
        else:
            return False

    # --------------------------------------------------------------------------------------------------------------
    #  Checks the all the rules in the concerned module and return the target object to append for pageType
    # --------------------------------------------------------------------------------------------------------------
    def getPageTypeRuleByName(self, markerManager, name):
        for rules in markerManager.getRules():
            if rules.type == "pageType" and rules.name == name:
                self.objectPageType = rules
                return True
        else:
            return False

    # --------------------------------------------------------------------------------------------------------------
    #  Return the instance of the VirtualMarker.markerQuery in order to update the targeted object
    # --------------------------------------------------------------------------------------------------------------
    def getSelectedRuleQuery(self, selectedObject):
        obj = selectedObject
        if not obj:
            return None
...
class RuleEditor(wx.Dialog):
    ...
    #------------------------------------------------------------------------------------------
    # Function returns the key of the current pageType
    #------------------------------------------------------------------------------------------
    def getCurrentContextPageType(self):
        # if rules.type == 'pageType' and rules.name == temp:
        return RuleContextEditor.getCurrentPageTypeName(self.data)
    ...
    #------------------------------------------------------------------------------------------
    # Update name value in pageType object while the contextPageType is changed in marker
    # object.
    # If contextPageType value exists in marker and it linked to the pageType name then update
    # -----------------------------------------------------------------------------------------
    def setPageTypeNameByObject(self, objMarkermanager, rule, data, name):

        data["name"] = name
        updateOrDeleteIfEmpty(data, "", "")
        layerName = None
        if rule is not None:
            layerName = rule.layer
        webModule = getEditableWebModule(objMarkermanager.webModule, layerName=layerName)
        if not webModule:
            return
        if rule is not None:
            objMarkermanager.removeRule(rule)
        if layerName == "addon":
            if not webModule.getLayer("addon") and webModule.getLayer("scratchpad"):
                layerName = "scratchpad"
        elif layerName is None:
        ...
        ruleManager = objMarkermanager
        webModule.createRule(data)
